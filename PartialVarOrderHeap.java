 ...
 */
public class PartialVarOrderHeap implements IOrder, Serializable {
	...
	private int[] partialVars;

	protected IPhaseSelectionStrategy phaseStrategy;

	public PartialVarOrderHeap() {
		this(new RSATPhaseSelectionStrategy());
	}

	public PartialVarOrderHeap(IPhaseSelectionStrategy strategy) {
		this.phaseStrategy = strategy;
	}
	
	public PartialVarOrderHeap(int[] partialVars) {
		this();
	 ...
	 */
	public int select() {
		if(varSelection < partialVars.length) {
			return LiteralsUtils.toInternal(partialVars[varSelection++]);
		}
		while (!heap.empty()) {
			int var = heap.getmin();
